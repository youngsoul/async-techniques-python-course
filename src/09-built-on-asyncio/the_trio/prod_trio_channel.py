...
"""
trio.Queue was deprecated for SendChannel and ReceiveChannel classes.

This example was updated to use the new channel implementation
...
async def generate_data(num: int, data: trio.abc.SendChannel):
...
async def process_data(num: int, data: trio.abc.ReceiveChannel):
    processed = 0
    while processed < num:
